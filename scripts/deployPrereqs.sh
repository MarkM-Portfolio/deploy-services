...
# docker_version=		# e.g. 17.03 or 18.06
...
# k8s_version=			# e.g. 1.11.1, 1.11.*, latest
...
k8s_cni_version=""
# Workaround for https://github.com/kubernetes/kubernetes/issues/75701
if [[ "${k8s_version}" =~ "1.11" ]]; then
        if [[ "${k8s_version}" != "1.11.9" && "${k8s_version}" != "1.11.*" ]]; then
                k8s_cni_version="0.6.*"
        fi
fi
...
function setup_registry(){
	...
	sudo docker run --entrypoint htpasswd registry:2 -Bbn admin password |  sudo tee /docker-registry/auth/htpasswd > /dev/null
	...
	sudo docker run -d -p 5000:5000 --restart=always --name registry -v /docker-registry/auth:/auth -v /docker-registry/certs:/certs -v /docker-registry/registry:/var/lib/registry -e "REGISTRY_AUTH=htpasswd" -e "REGISTRY_AUTH_HTPASSWD_REALM=Registry Realm" -e "REGISTRY_AUTH_HTPASSWD_PATH=/auth/htpasswd" -e "REGISTRY_HTTP_TLS_CERTIFICATE=/certs/cert.pem" -e "REGISTRY_HTTP_TLS_KEY=/certs/key.pem" registry:2
...
function setup_pvs() {
	...
	LIST="connections-persistent-storage-nfs-0.1.0.tgz nfsSetup.sh"
	...
	sudo tar -xvzf connections-persistent-storage-nfs-0.1.0.tgz
...
function docker_install() {
	...
	if [ "${docker_version}" = "17.03" ]; then
		# Stop docker
		sudo systemctl stop docker

		# Ensure the required sysctl knob setting which controls kernel behavior is set (17.03 only - set by default with 18.06)
		if [ -e /proc/sys/fs/may_detach_mounts ]; then
			echo 1 | sudo tee /proc/sys/fs/may_detach_mounts > /dev/null
			echo fs.may_detach_mounts=1 | sudo tee /usr/lib/sysctl.d/99-docker.conf > /dev/null
		fi
...
function create_primary_config_yaml () {
	...
	if [ ${comparison_result} = 1 -o ${comparison_result} = 0 ]; then
		...
		echo "apiVersion: kubeadm.k8s.io/v1beta1" | sudo tee /${homeFolder}/kubeadm-config.yaml > /dev/null
...
if [ ${create_pvs} = true ]; then
	if [ "${external_nfs_server}" = "" ]; then
		...
		sudo rm -rf connections-persistent-storage-nfs connections-persistent-storage-nfs-0.1.0.tgz volumes.txt
		...
		sshpass -p ${ssh_password} ssh -o ConnectTimeout=1 -o UserKnownHostsFile=/dev/null -o LogLevel=quiet -o StrictHostKeyChecking=no -n ${USER}@${external_nfs_server} "sudo rm -rf connections-persistent-storage-nfs connections-persistent-storage-nfs-0.1.0.tgz volumes.txt"
	...
	if [ "${external_nfs_server}" != "" ]; then
		LIST="connections-persistent-storage-nfs-0.1.0.tgz"
	...
	helm install --name=connections-volumes connections-persistent-storage-nfs-0.1.0.tgz --set nfs.server=${nfs_server},persistentVolumePath=${pvconnections_folder_path}
fi
