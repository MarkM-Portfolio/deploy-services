#!/bin/bash -
#title           :solr-nfs-volumes-creation.sh
#description     :This script must be used only in test environments.
#                 It will create PVs and PVCs to be used by Solr in K8s cluster on CFC.
#version         :0.1
#usage		       :solr-nfs-volumes-creation.sh
#=================================================================================================
#!/bin/bash
...
usage() {
  logIt ""
  logIt "Usage: ./solr-nfs-volumes-creation.sh [OPTION]"
  logIt "Must not be used in production. It will do:"
  logIt "Install and set up a basic NFS server configuration"
  logIt "Create the pv.yml and pvc.yml of: https://github.ibm.com/connections/deploy-services/tree/master/microservices/hybridcloud/templates/solr"
  logIt ""
...
force() {
  ...
  while [ $i -lt $ATTEMPTS ]
  do
    PVCs_BOUND_N=$(kubectl get -f pvc.yml | awk '{ print $2 }' | grep -v Bound | wc -l)
...
downloadYmls() {
  ...
  OWNER="connections"
  REPO="solr-basic"
  PATH_FILE="deployment/infrastructure/pv.yml"
  FILE="https://github.ibm.com/api/v3/repos/$OWNER/$REPO/contents/$PATH_FILE"
  curl -H "Authorization: token $TOKEN" \
  -H "Accept: application/vnd.github.v3.raw" \
  -O \
  -L $FILE
  PATH_FILE="deployment/infrastructure/pvc.yml"
  FILE="https://github.ibm.com/api/v3/repos/$OWNER/$REPO/contents/$PATH_FILE"
  curl -H "Authorization: token $TOKEN" \
  -H "Accept: application/vnd.github.v3.raw" \
  -O \
